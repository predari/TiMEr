...
############# IGNORE COLLECTION ############
##ALLDATA <- ALLDATA[ALLDATA$COLLECTION != "dimacs", ] # ignore dimacs
ALLDATA <- ALLDATA[ALLDATA$COLLECTION != "network", ] # ignore network
...
MEANDATA <- ddply(NORMDATA,
                  .(FILE, COLLECTION, METHOD, TOPOLOGY), # compute separate mean for each FILE x COLLECTION x METHOD x NPARTS
                  summarise,
...
############ DUMP RESULTS ############
MEANDATA[c('FILE','METHOD','TOPOLOGY','CUT','CUTSD','DURATION','DURATIONSD')] 
...
cutplot <- ggplot(MEANDATA, aes(x=TOPOLOGY, y=CUT, fill=METHOD, colour=METHOD, group=METHOD)) +
    ...
    facet_wrap( ~ FILE, ncol=2, scales="fixed") +    
