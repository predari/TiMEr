...
#define forall_out_edges_in_random_order(G,e,n) { EdgeID e; std::vector<EdgeID> out_edges;  forall_out_edges(G, e_bar, n ) { out_edges.push_back(e_bar); } endfor random_functions::permutate_vector_good(out_edges,false); for(unsigned i = 0, end = out_edges.size(); i < end; ++i) { e = out_edges[i];
...
class graph_access {
        ...
        public:
                ...
                EdgeWeight getMinDegree();//djoko
                EdgeWeight getMaxDegree();
                EdgeWeight getMinDegreeUnweighted();//djoko
                EdgeWeight getMaxDegreeUnweighted();//djoko
		EdgeWeight getNumberSelfLoops();//djoko
		EdgeWeight getNumberAdditionalParallelEdges();//djoko
...
/******************************* djoko ******************************/
inline EdgeWeight graph_access::getMinDegree(){
  EdgeWeight ret = INT_MAX;
  basicGraph& ref = *graphref;
  forall_nodes(ref, node){
    EdgeWeight cur_degree = 0;
    forall_out_edges(ref, e, node){
...
inline EdgeWeight graph_access::getMinDegreeUnweighted(){
  EdgeWeight ret = INT_MAX;
  basicGraph& ref = *graphref;
  forall_nodes(ref, node){
...
inline EdgeWeight graph_access::getMaxDegreeUnweighted(){
  EdgeWeight ret = 0;
  basicGraph& ref = *graphref;
  forall_nodes(ref, node){
    EdgeWeight cur_degree = getNodeDegree(node);
...
inline EdgeWeight graph_access::getNumberSelfLoops(){
  EdgeWeight ret = 0;
  basicGraph& ref = *graphref;
  forall_nodes(ref, node){
    forall_out_edges(ref, e, node){
      if(getEdgeTarget(e) == node){
	ret++;
      }
    } endfor
  } endfor
...
inline EdgeWeight graph_access::getNumberAdditionalParallelEdges(){
  EdgeWeight ret = 0;
  basicGraph& ref = *graphref;
  forall_nodes(ref, node){
    std::priority_queue<NodeID> pq;
    forall_out_edges(ref, e, node){
      pq.push(getEdgeTarget(e));
    } endfor
    if(pq.empty() == false){
      NodeID oldId = pq.top();
      pq.pop();
