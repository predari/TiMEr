/*
 * copy&pasted and then slightly modified from KaHIPv0.62/lib/partition/uncoarsening.cpp
 */

//TODO: call diffusive_refinement
...
diffusive_uncoarsening::diffusive_uncoarsening() {
...
int diffusive_uncoarsening::perform_uncoarsening(const PartitionConfig & config, graph_hierarchy & hierarchy) {
        int improvement = 0;
	...
	//currently no member to specify diffusive_refinement in PartitionConfig
	//so for the moment always use it
	//TODO: correct this
	refinement* diff_refine = new diffusive_refinement();
        ...
        improvement += (int)diff_refine->perform_refinement(cfg, *coarsest, *coarser_boundary);
	//improvement += (int)refine->perform_refinement(cfg, *coarsest, *coarser_boundary);
		...
		improvement += (int)diff_refine->perform_refinement(cfg, *G, *finer_boundary);
