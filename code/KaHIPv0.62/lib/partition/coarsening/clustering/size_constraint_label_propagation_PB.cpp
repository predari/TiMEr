...
void size_constraint_label_propagation::label_propagation(const PartitionConfig & partition_config, 
        ...
        std::vector<EdgeRatingType> connectivityByEdgeRating(G.number_of_nodes(), 0);
        ...
        for( int j = 0; j < partition_config.label_iterations; j++) {
                ...
                forall_nodes(G, i) {
                        ...
                        forall_out_edges(G, e, node) {
                                ...
                                connectivityByEdgeRating[cluster_id[target]] += G.getEdgeRating(e);
                        ...
                        EdgeRatingType max_connectivity = 0;
                        ...
                        forall_out_edges(G, e, node) {
                                ...
                                EdgeRatingType cur_edgeRating = connectivityByEdgeRating[cur_block];
...
//                                }
                                ...
                                if((/*cur_value > max_value  || (cur_value == max_value &&*/ (cur_edgeRating > max_connectivity) || (fabs(cur_edgeRating - max_connectivity) < 0.0000001 && random_functions::nextBool())) 
                                && (cluster_sizes[cur_block] + G.getNodeWeight(node) < block_upperbound || cur_block == my_block) 
                                && (!partition_config.graph_allready_partitioned || G.getPartitionIndex(node) == G.getPartitionIndex(target))
                                && (!partition_config.combine || G.getSecondPartitionIndex(node) == G.getSecondPartitionIndex(target)))
                                {
                                        max_value = cur_value;
                                        max_connectivity = cur_edgeRating;
                                        max_block = cur_block;
                                }

                                hash_map[cur_block] = 0;
                                connectivityByEdgeRating[cur_block] = 0;
                        } endfor
