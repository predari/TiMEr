...
void prepPartitioning(PartitionConfig & partition_config, graph_access & G, bool repartition) {
  ...
  if(  partition_config.imbalance == 0 ) {
    ...
    //RG: first term on RHS = 1.01. Why is this perfect balance?
  ...
  partition_config.graph_allready_partitioned = repartition; //RG always false
  partition_config.kway_adaptive_limits_beta  = log(largest_graph_weight); //RG: what is this?
...
void repartition(graph_access& G, int k, int imbalance, int mode) {
  ...
  prepPartitioning(repartition_config, G, false); //RG: parameter "repartition" always false? 
...
int main(int argn, char** argv) {
  ...
  //RG: Does this have any effect?

  struct arg_str *g_filename = arg_strn(NULL, NULL, "FILE", 1, 1, "Path to graph file to partition.");
  //RG: why not using arg_str1?
  //RG: why not = "g_filename->sval[0] = "../graph_part_viz/samplegraphs/Grid100x100";"
  //instead of "file = g_filename->sval[0];" later on?

  struct arg_dbl *g_genImbalance = arg_dbl0(NULL, "genImbalance", NULL, "Desired balance. Default: 20 (%).");
  //RG: why not "g_genImbalance->dval[0] = 20.0;"  instead of int "genImbalance = 20;" below?
  ...
  //RG: If seq == false and gen == false, we have no help even if sg_help->count > 0, right?   

  int k = 16;
  int imbalance = 3; //Why int?
  int mode = STRONG;
  const char* file_out = "tmppartition";
  //RG: default values should be set above 
  ...
  if(sg_preconfiguration->count > 0) {
    if(strcmp("strong", sg_preconfiguration->sval[0]) == 0) {
      ...
      exit(0); //RG: Shouldn't the default be strong (see above)?
    }
  }
