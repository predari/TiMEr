...
bool map(alma_graph_access & g, graph_access & C, processor_graph_access & P,
  ...
  bool OK = true;
  // maria following line for alma output
  int64_t initialCut, finalCut, initialTCV, finalTCV;
		...
		if(mappingAlgo == "brandfassL") {
		  ...
		  if(mappingAlgo == "hoefler") {
		    ...
		    if(mappingAlgo == "hoeflerL") {
		      ...
		      if(mappingAlgo == "alma") {
			almaMapping(g, P, map_config, mapping, initialCut, finalCut, initialTCV, finalTCV);
			printf("almaMapping was done successfully!\n"); /* maria */
		      } else {
...
bool almaMapping(alma_graph_access & g, processor_graph_access & P,
  ...
  //First, set vlabels[v] to block ID of v
  ...
  g.makeLabel2Vertex(g);

  //g.display();
  //g.statisticsOnMissfits();

  //g.checkPartition(g, map_config.k);
  initialCut = g.getTotalCut(g);
  initialTCV = g.getTotalCommunicationCost(g);
  int64_t maxTCV = g.getMaxCommunicationCost(g);
  cout << ":::Application Graph --> meanDegree: " << g.getMeanDegree(g) << "   minDegree:" << g.getMinDegree(g) << "   maxDegree:" << g.getMaxDegree(g) << "\n";
  cout << " mapping 1: max_comm_vol is " << maxTCV << endl;
  //cout << "Cut and total communication costs are " <<  initialCut << " and " << initialTCV << endl;
  //cout << endl;

  int64_t newTCV = initialTCV;
    
  //Loop over all hierarchies
  for(int64_t run = 0; run < map_config.numberHierarchies; run++) {
    int64_t oldTCV = newTCV;

    //save the Hamming labels of g, just in case the label swapping makes things worse
    vector<int64_t> oldLabelsG(g.number_of_nodes());
    forall_nodes(g, v) {
      oldLabelsG[v] = g.getLabel(v);
    } endfor
    ...
    while(digit > 0) {//loop over graphs in hierarchy
      if(ip.entry(digit) >= g.getNumDigitsBlockID()) {
	newG_ptr->swapVertexLabelsDiversification(*newG_ptr, digit);
      } else {
	newG_ptr->swapVertexLabelsIntensification(*newG_ptr, digit);
      }
    ...
    //check whether label swapping was successful
    newTCV = g.getTotalCommunicationCost(g);
    if(newTCV > oldTCV) {//failure: swapping made things worse, reset labels of g
      forall_nodes(g, v) {
	g.setLabel(v, oldLabelsG[v]);
      } endfor
      g.makeLabel2Vertex(g);
      newTCV = oldTCV;
    }
    ...
    //g.checkPartition(g, map_config.k);
    //cout << "Cut and  total communication costs are " <<  finalCut << " and " << finalTCV << endl;
    //cout << endl;
  }
  finalCut = g.getTotalCut(g);
  finalTCV = newTCV;
  int64_t finalmaxTCV = g.getMaxCommunicationCost(g);
  cout << " mapping 2: max_comm_vol is " << finalmaxTCV << endl;
  return(true);
}
