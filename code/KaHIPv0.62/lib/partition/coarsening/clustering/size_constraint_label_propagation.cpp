...
void size_constraint_label_propagation::label_propagation(const PartitionConfig & partition_config, 
  ...
  std::vector<PartitionID> sumEdgeWeights(G.number_of_nodes(),0);
  ...
  for( int j = 0; j < partition_config.label_iterations; j++) {
	...
	sumEdgeWeights[cluster_id[target]]+=G.getEdgeWeight(e);
      } endfor
      ...
      EdgeRatingType max_value = 0.0;
      forall_out_edges(G, e, node) {
	NodeID target             = G.getEdgeTarget(e);
	PartitionID cur_block     = cluster_id[target];
	EdgeRatingType cur_value  = ((EdgeRatingType)(sumEdgeWeights[cur_block])) + G.getEdgeRating(e);
