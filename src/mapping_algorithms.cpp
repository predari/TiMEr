...
/*                        map                         */
/******************************************************/
void map(alma_graph_access & g, graph_access & C, processor_graph_access & P, PartitionConfig & map_config,
	  ...
	  if(mappingAlgo == "multikahip") {
		...
		if(mappingAlgo == "hoefler") {
		  ...
		  if(mappingAlgo == "hoeflerL") {
		    ...
		    if(mappingAlgo == "alma") {
		      almaMapping(g, C, P, map_config, numberHierarchies, mapping);
		    } else {
...
/*                                                                 */
/*******************************************************************/
void almaMapping(alma_graph_access & g, graph_access & C, processor_graph_access & P,
  ...
  alma_graph_access alma_proc_graph;
      //bool OK = P.getLabelsPartialCube();
      //if(OK == false) {
      //return();
      //}
}
//                 for(int64_t v = 0; v < g.getNumVertices(); v++) {
//                     int64_t vlab = gp->getLabel(g.getLabel(v));
//                     cout << "vlab is " << vlab << endl;
//                     g.setLabel(v, vlab);
//                 }
//             }
//             //At this point, the label of a vertex of g indicates the Hamming label of the vertice's block
//             //Note that after readPartition, the label of a vertex of g indicates the block ID
//             g.getVertexLabelsFromPartition();
//             for(int64_t v = 0; v < g.getNumVertices(); v++) {
//                 cout << "Label of vertex " << v << " is " << g.getLabel(v) << endl;
//             }
//         }else {
//             if(argc < 4) cout << "Too few parameters" << endl;
//             if(argc > 6) cout << "Too many parameters" << endl;
//             return(0);
//         }
//     }
//     g.makeVectorLabel2Vertex();
//     //g.display();
//     //g.statisticsOnMissfits();
//     for(int64_t run = 0; run < numberHierarchies; run++) { //loop over all hierarchies
...
//         cout << "1:::Total communication costs and total cut are " << g.getTotalCommunicationCost() <<
//         " and " << g.getTotalCut() << endl;
        
//         g.permuteVertexLabels(pi);
...
//                 newg.swapVertexLabelsDiversification();
//             } else {
//                 newg.swapVertexLabelsIntensification();
//             }
//             hi.insertGraph(newG, level);
//             vector<int64_t> parents(0);
//             newG = newg.contract(newG, parents);
//             hi.insertParents(parents, level);
//             digit--;
//             level++;
//         }
//         hi.updateLabelsOnFinestLevel();
//         g.permuteVertexLabels(ip);
//         cout << "2:::Total communication costs and total cut are " << g.getTotalCommunicationCost() <<
//         " and " << g.getTotalCut() << endl;
//         cout << endl;
//     }
//     return 0;
